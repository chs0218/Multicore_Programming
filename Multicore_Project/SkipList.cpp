...
이 구조를 사용하기 위해서는 메모리 재사용을 위해 shared_ptr, EBR, Hazard Pointer 등을 사용해야한다.
EBR같은 재사용이 들어가려면 Add나 Remove할 때, 포인터를 잘 설정해줘야한다.

게으른 동기화가 Lock-Free보다 더 좋게 나올 수 있다. 이는 CAS의 반복 사용으로 인한 오버헤드 때문.
하지만, 총돌이 많이 일어나는 환경의 경우에 게으른 동기화가 성능 저하가 심하다.

충돌이 심하지 않은 경우에는 게으른 동기화가 성능이 좋을 수 있지만 게으른 동기화를 쓰는것이 좋다라고 할 수 없다.
충돌이 심한 환경이 되면 Lock-Free는 성능 저하가 심하지 않지만 게으른 동기화는 성능 저하가 심하다.
=> 그래서 우리는 Lock-Free를 써야한다.

Lock-Free는 자료구조의 특성에 맞게 최적화 하는게 어렵다.
*/
...
public:
	...
	bool Contains(int key)
	{
		...
		for (int i = TOP_LEVEL; i >= 0; --i) {
			...
			while (1) {
				bool removed = false;
				LFSKNODE* succ = curr->Get(i, &removed);
